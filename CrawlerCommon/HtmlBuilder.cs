...
namespace CrawlerCommon
{
    static public class HtmlBuilder
    {
        static public IEnumerable<Token> BuildTree(this IEnumerable<string> list, DefaultBuilder builder) 
        {
            ...
            {
                var hack= item.ToLower(); //hack to detect <script>, and ignore everything tokenizer (probably incorrectly) tokenized
                if (!inscript && hack.StartsWith("<script ") && hack.EndsWith(">"))
                {
                    inscript = true;
                    //yield return new ScriptTag(null, item);
                    Token found = builder.Construct(item);
                    if (found != null)
                        yield return found;
                }
                else if (inscript && hack == "</script>")
                {
                    inscript = false;
                    //yield return new Closure(null, item);
                }
                if (!inscript)
                {
                    Token found = builder.Construct(item);
        ...
        static public IEnumerable<Token> BuildTreeFrom(this Page page,string url, DefaultBuilder builder)
        {
            var inscript = false;
            //DefaultBuilder builder = new DefaultBuilder();
            foreach (var item in page.StreamIn(url))
            {
                var hack = item.ToLower(); //hack to detect <script>, and ignore everything tokenizer (probably incorrectly) tokenized
                if (!inscript && hack.StartsWith("<script ") && hack.EndsWith(">"))
                {
                    ...
                    if (found != null)
                        yield return found;
                    page.TokenizerControl = CloseScriptDetector.Singleton.Check;
                }
    ...
    public class CloseScriptDetector
    {
        static public CloseScriptDetector Singleton = new CloseScriptDetector();

        char[] _buffer = new char[9];
        int _end = 0;
        public string Check(char ch)
        {
            int ndx = 0;
            _buffer[_end++] = char.ToLower(ch);
            if (_end >= 9) _end = 0;
            if (_buffer[(ndx = (_end - 1)) < 0 ? 9 + ndx : ndx] != '>') return null;
            if (_buffer[(ndx = (_end - 2)) < 0 ? 9 + ndx : ndx] != 't') return null;
            if (_buffer[(ndx = (_end - 3)) < 0 ? 9 + ndx : ndx] != 'p') return null;
            if (_buffer[(ndx = (_end - 4)) < 0 ? 9 + ndx : ndx] != 'i') return null;
            if (_buffer[(ndx = (_end - 5)) < 0 ? 9 + ndx : ndx] != 'r') return null;
            if (_buffer[(ndx = (_end - 6)) < 0 ? 9 + ndx : ndx] != 'c') return null;
            if (_buffer[(ndx = (_end - 7)) < 0 ? 9 + ndx : ndx] != 's') return null;
            if (_buffer[(ndx = (_end - 8)) < 0 ? 9 + ndx : ndx] != '/') return null;
            if (_buffer[(ndx = (_end - 9)) < 0 ? 9 + ndx : ndx] != '<') return null;
